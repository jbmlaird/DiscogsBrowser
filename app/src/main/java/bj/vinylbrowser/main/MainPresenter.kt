 ...
 */
class MainPresenter @Inject constructor(val context: Context, val mView: MainContract.View, val discogsInteractor: DiscogsInteractor,
                                        val mySchedulerProvider: MySchedulerProvider, val builder: NavigationDrawerBuilder, val controller: MainEpxController,
                                        val sharedPrefsManager: SharedPrefsManager, val log: LogWrapper, val daoManager: DaoManager,
                                        val tracker: AnalyticsTracker) : MainContract.Presenter {
    ...
    override fun connectAndBuildNavigationDrawer(toolbar: Toolbar?) {
        ...
        fetchMainPageInformation()
                .subscribe({ listing ->
                    ...
                    mView.setDrawer(builder.buildNavigationDrawer(mView.activity as FirstActivity?, toolbar))
                },
                        { error ->
